...
export default function (data) {
	...
	check(csrfResponse, {
		...
		"csrf response has csrf_token": (r) => {
			try {
				const body = JSON.parse(r.body);
				// biome-ignore lint/complexity/useOptionalChain: <explanation>
				return body.data && body.data.csrf_token;
			} catch (e) {
				return false;
			}
		},
	...
	if (csrfResponse.status === 200) {
		...
		console.error("CSRF request failed with status:", csrfResponse.status);
		return;
	}
